...
# Testing
# group_messages = {}
# JAINA_GROUP_IDS = {-4592052778, -4580409570, -4577156289}
# OUR_BETS_GROUP_IDS = {-4535798028} 
...
#     for line in lines:
#         line = line.strip().strip('/n')  # Remove leading/trailing whitespace
...
#             print(f"Detected bet ID: {current_bet_id}")
            
#         elif ':' in line and '@' not in line:
#             current_bet_type = line.strip().lower().replace(':', '').strip()
#             print(f"Detected bet type: {current_bet_type}")
...
#                 odds = float(parts[1])
#                 bet_type = 'Jaina' if 'jaina' in current_bet_type else 'our_bets'
#                 print(f"Adding bet - Type: {bet_type}, Amount: {amount}, Odds: {odds}")
#                 if current_bet_id:
...
#     compiled_messages = {'jaina': [], 'our_bets': []}
    
#     # Debug: Print command text and parts
#     print(f"Command text: {command_text}")
#     print(f"Parts: {parts}")
    
#     # Iterate over each part of the command text
#     for part in parts:
#         part = part.strip()
...
#                 # Check if the message has a 'text' field and it is not None
#                 if message.get('text') and part.lower() in message['text'].lower():
#                     print(f"Match found: '{message['text']}'")
#                     print(f"Group ID: {group_id}, JAINA_GROUP_IDS: {JAINA_GROUP_IDS}")
...
#             response += f"{msg}\n\n"
#         print("Jaina messages compiled.")
    
#     if compiled_messages['our_bets']:
#         response += "\nour bets:\n"
#         for msg in compiled_messages['our_bets']:
#             response += f"{msg}\n\n"
#         print("Our Bets messages compiled.")

#     # Process the compiled messages similarly as before
...
#     # application = Application.builder().token('7235106922:AAGZBS6jciEM3TJ-z5J1TgobICidM-laTeA').build()
...
#     ))
#     application.add_handler(MessageHandler(
#         filters.Chat(chat_id=-4261988114) & (~filters.UpdateType.EDITED_MESSAGE),
#         group_message_handler
#     ))
